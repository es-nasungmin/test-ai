...
namespace AiDeskApi.Services
{
    ...
    public class KnowledgeBaseWriterPromptTemplateService : IKnowledgeBaseWriterPromptTemplateService
    {
        ...
        private const string DefaultKeywordRulesPrompt = "1) 핵심 기능/대상/증상/원인 중심의 구체 키워드를 우선한다\n2) 사용자 검색어 관점(실제 문의 표현)과 도메인 용어를 함께 반영한다\n3) 중복/유사 표현은 통합한다\n4) 너무 포괄적인 단어(예: 오류, 문제, 문의)는 단독으로 사용하지 않는다";
        private const string DefaultExpectedQuestionSystemPrompt = "당신은 고객 문의 패턴 설계 도우미입니다. KB 내용을 바탕으로 실제 사용자가 입력할 다양한 질문 표현을 생성해 검색 타겟팅 확률을 높입니다.";
        private const string DefaultExpectedQuestionRulesPrompt = "1) 각 질문은 실제 사용자 말투로 짧고 자연스럽게 작성한다\n2) 같은 의미라도 표현/어순/어휘를 바꿔 다양화한다\n3) 핵심 증상, 상황, 실패 지점을 반영해 타겟팅 범위를 넓힌다\n4) 문서 근거 밖 내용은 만들지 않는다\n5) 제목 문장을 그대로 복사하지 않는다";
        private const string DefaultTopicKeywordSystemPrompt = "당신은 고객센터 KB 분류 도우미입니다. 답변 내용을 기준으로 주제와 도메인 키워드를 추출합니다.";
        private const string DefaultTopicKeywordRulesPrompt = "1) 도메인 용어와 주제 분류에 중점을 둔다\n2) 다른 KB와의 관련도 연결에 도움이 되는 키워드를 우선한다\n3) 너무 일반적인 단어는 제외한다";
            ...
            string answerRefineRulesPrompt)
        {
            ...
            if (string.IsNullOrWhiteSpace(expectedQuestionSystemPrompt)) throw new ArgumentException("예상질문 시스템 프롬프트는 비울 수 없습니다.");
            if (string.IsNullOrWhiteSpace(expectedQuestionRulesPrompt)) throw new ArgumentException("예상질문 규칙 프롬프트는 비울 수 없습니다.");
            ...
            entity.ExpectedQuestionSystemPrompt = expectedQuestionSystemPrompt.Trim();
            entity.ExpectedQuestionRulesPrompt = expectedQuestionRulesPrompt.Trim();
        ...
        {
            var entity = await _context.KnowledgeBaseWriterPromptTemplates.OrderBy(x => x.Id).FirstOrDefaultAsync();
            if (entity != null)
            {
                var changed = false;

                changed |= NormalizeAndAssign(entity.KeywordSystemPrompt, DefaultKeywordSystemPrompt, value => entity.KeywordSystemPrompt = value);
                changed |= NormalizeAndAssign(entity.KeywordRulesPrompt, DefaultKeywordRulesPrompt, value => entity.KeywordRulesPrompt = value);
                changed |= NormalizeAndAssign(entity.ExpectedQuestionSystemPrompt, DefaultExpectedQuestionSystemPrompt, value => entity.ExpectedQuestionSystemPrompt = value);
                changed |= NormalizeAndAssign(entity.ExpectedQuestionRulesPrompt, DefaultExpectedQuestionRulesPrompt, value => entity.ExpectedQuestionRulesPrompt = value);
                changed |= NormalizeAndAssign(entity.TopicKeywordSystemPrompt, DefaultTopicKeywordSystemPrompt, value => entity.TopicKeywordSystemPrompt = value);
                changed |= NormalizeAndAssign(entity.TopicKeywordRulesPrompt, DefaultTopicKeywordRulesPrompt, value => entity.TopicKeywordRulesPrompt = value);
                changed |= NormalizeAndAssign(entity.AnswerRefineSystemPrompt, DefaultAnswerRefineSystemPrompt, value => entity.AnswerRefineSystemPrompt = value);
                changed |= NormalizeAndAssign(entity.AnswerRefineRulesPrompt, DefaultAnswerRefineRulesPrompt, value => entity.AnswerRefineRulesPrompt = value);
            ...
            entity = new KnowledgeBaseWriterPromptTemplate
            {
                ...
                ExpectedQuestionSystemPrompt = DefaultExpectedQuestionSystemPrompt,
                ExpectedQuestionRulesPrompt = DefaultExpectedQuestionRulesPrompt,
        ...
        private static bool NormalizeAndAssign(string current, string fallback, Action<string> assign)
        {
            var normalized = string.IsNullOrWhiteSpace(current) ? fallback : NormalizeLineBreaks(current);
        ...
        private static KnowledgeBaseWriterPromptTemplateSnapshot ToSnapshot(KnowledgeBaseWriterPromptTemplate entity)
        {
            return new KnowledgeBaseWriterPromptTemplateSnapshot(
                ...
                entity.ExpectedQuestionSystemPrompt,
                entity.ExpectedQuestionRulesPrompt,
