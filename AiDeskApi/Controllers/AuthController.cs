...
namespace AiDeskApi.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<ActionResult<UserDto>> GetMe()
        {
            ...
            var user = await _context.Users
                .AsNoTracking()
                .FirstOrDefaultAsync(u => u.Id == userId && u.Status != "deleted");
        ...
        public async Task<ActionResult<LoginResponse>> UpdateMyProfile([FromBody] UpdateMyProfileRequest request)
        {
            ...
            var user = await _context.Users.FirstOrDefaultAsync(u => u.Id == userId && u.Status != "deleted");
            if (user == null)
            {
                return NotFound(new LoginResponse
                {
        ...
        public async Task<ActionResult<LoginResponse>> UpdateUser(int userId, [FromBody] UpdateUserByAdminRequest request)
        {
            ...
            if (hasStatus && status == null)
            {
                ...
                {
                    Success = false,
                    Message = "상태는 approved, pending, deleted 중 하나여야 합니다."
                });
            }
        ...
        public async Task<ActionResult<LoginResponse>> RejectUser(int userId)
        {
            ...
            // 거절 = pending으로 상태 복귀
            ApplyUserStatus(userToReject, "pending");
            await _context.SaveChangesAsync();

            return Ok(new LoginResponse
            {
                Success = true,
                Message = $"{userToReject.Username} 사용자 승인을 거절했습니다."
            });
        }
        ...
        private static string? NormalizeUserStatus(string? status)
        {
            ...
            if (normalized == "approved" || normalized == "pending" || normalized == "deleted")
            {
                return normalized;
            }
            return null;
        ...
        {
            var status = user.Status?.Trim().ToLowerInvariant() ?? "pending";
            return status == "approved" || status == "deleted" ? status : "pending";
        }
